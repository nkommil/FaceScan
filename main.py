...
face_dir = 'haar_faces'
detected_dir = 'haar_detected'
path = os.path.dirname(__file__)

def create_dir():
    dirs = [face_dir, detected_dir]
    for dir in dirs:
...
def haar_detected(image):
    ...
    if len(faces):
        ...
        cv2.imwrite(os.path.join(path, detected_dir, name+'detect_face.jpg'), copy_img)
        return copy_img
